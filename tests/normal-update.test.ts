...
const releaseFixturePath = fileURLToPath(new URL("../../tests/fixtures/releases/development-normal.json", import.meta.url));
...
function descriptor(version: string, channel: "stable" | "preview" | "development" = "development"): ReleaseDescriptor {
  return { version, channel, projectBrainSchema: 1, compatibility: { from: ["0.0.0-development"] }, sourceId: "official-local-test-source", artifact: { id: "runtime-node-cli", sha256: normalDigest } };
}
function installedRelease(fixture: RuntimePackageFixture): ReleaseDescriptor {
  return { version: targetVersion, channel: "development", projectBrainSchema: 1, compatibility: { from: ["0.0.0-development"] }, sourceId: "official-local-test-source", artifact: { id: "runtime-node-cli", sha256: fixture.sha256 } };
}
...
async function loadReleases(): Promise<ReleaseDescriptor[]> { return JSON.parse(await readFile(releaseFixturePath, "utf8")) as ReleaseDescriptor[]; }
