...
function snapshot(total: { inputTokens: number; cachedInputTokens: number; outputTokens: number; reasoningOutputTokens: number; totalTokens: number; cacheWriteInputTokens?: number }, threadId = "thread-1"): CodexUsageSnapshot {
  return { source: { kind: "provider-runtime", id: "openai.codex.app-server.thread-token-usage", version: "0.142.4" }, threadId, turnId: "turn-1", last: { inputTokens: 1, cachedInputTokens: 0, outputTokens: 1, reasoningOutputTokens: 0, totalTokens: 2 }, total };
}

test("external/resumed Codex usage baselines first, deduplicates and re-baselines regressions", () => {
  ...
  if (result.kind === "delta") assert.deepEqual(result.event.usage, { inputTokens: 30, cacheReadTokens: 10, outputTokens: 7, reasoningTokens: 3, totalTokens: 40 });
...
test("a thread successfully created by Livariant may use zero as trustworthy baseline", () => {
  const sequencer = new CodexUsageSequencer();
  sequencer.markNewThread("new-thread");
  const result = sequencer.accept(snapshot({ inputTokens: 12, cachedInputTokens: 3, outputTokens: 4, reasoningOutputTokens: 1, totalTokens: 17 }, "new-thread"));
  assert.equal(result.kind, "delta");
  if (result.kind === "delta") assert.deepEqual(result.event.usage, { inputTokens: 12, cacheReadTokens: 3, outputTokens: 4, reasoningTokens: 1, totalTokens: 17 });
});
...
test("provider-owned Codex deltas persist canonically without raw content and aggregate as Observed", async () => {
  ...
  try {
    ...
    const sequencer = new CodexUsageSequencer();
    sequencer.markNewThread("thread-1");
    ...
    assert.equal(aggregate.observed.inputTokens, 32);
    assert.equal(aggregate.observed.cacheReadTokens, 7);
    assert.equal(aggregate.observed.outputTokens, 8);
    assert.equal(aggregate.observed.reasoningTokens, 2);
    assert.equal(aggregate.observed.totalTokens, 40);
