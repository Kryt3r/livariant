...
test("Settings-only rerenders rebind the update check action", async () => {
  ...
  assert.match(main, /renderSettingsSectionOnly[sS]*bindUpdateCheckEvent();[sS]*bindConnectionDiagnosticsEvents/);
  assert.match(main, /const bindEvents = () => {[sS]*bindUpdateCheckEvent();/);
