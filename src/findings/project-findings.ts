...
import { buildBootstrapDiscovery } from "../project/bootstrap-discovery.js";
...
function isRegularNonSymlinkFile(path: string, maxBytes?: number): boolean {
  try {
    ...
    return stat.isFile() && !stat.isSymbolicLink() && (maxBytes === undefined || stat.size <= maxBytes);
  } catch {
    return false;
  }
}
...
function inspectBootstrapAttention(root: string): ProjectFinding[] {
  const discovery = buildBootstrapDiscovery(discoverProject(root));
  const findings: ProjectFinding[] = [];

  for (const item of discovery.attention) {
    if (item.code === "discovery-unsafe-package-manifest") {
      findings.push(finding({
        ...
        evidence: item.provenance.map((path) => ({ path, detail: "not a regular non-symlink file" })),
    ...
    } else if (item.code === "discovery-unreadable-package-manifest") {
      findings.push(finding({
        ...
        evidence: item.provenance.map((path) => ({ path, detail: "JSON parsing failed" })),
        nextStep: "Validate and repair package.json before relying on package, dependency, or script automation.",
      }));
    } else if (item.code === "discovery-multiple-node-lockfiles") {
      findings.push(finding({
        ...
        evidence: item.provenance.map((path) => ({ path, detail: "package-manager lockfile present" })),
...
function inspectPackageManifest(root: string): ProjectFinding[] {
  ...
  if (!existsSync(packagePath) || !isRegularNonSymlinkFile(packagePath, MAX_PACKAGE_BYTES)) return [];
...
function inspectSensitiveRepositoryRoot(root: string): ProjectFinding[] {
  ...
  if (present.length === 0 || existsSync(resolve(root, ".gitignore"))) return [];
  ...
  return [finding({
    ...
    severity: "high",
    confidence: "moderate",
    title: "Sensitive root file has no visible repository-local ignore policy",
    explanation: "A commonly sensitive file is present in a Git workspace and no .gitignore exists at the project root. This does not prove the file is committed or exposed, but it removes a common repository-local guard against accidental inclusion.",
    evidence: present.map((path) => ({ path, detail: "sensitive filename present; file contents were not read" })),
    nextStep: "Verify whether the file is tracked or staged, confirm the repository's intended ignore policy, and rotate any secret that may already have been exposed.",
  })];
}
...
export function scanProjectFindings(projectPath: string = process.cwd()): ProjectFindingsReport {
  const root = discoverProject(projectPath).root;
  const findings = [
    ...inspectBootstrapAttention(root),
    ...inspectPackageManifest(root),
    ...inspectSensitiveRepositoryRoot(root),
    ...inspectAgentGuidance(root),
  ].sort((a, b) => `${a.severity}:${a.ruleId}:${a.id}`.localeCompare(`${b.severity}:${b.ruleId}:${b.id}`));
