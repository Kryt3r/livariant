...
export type ProjectInspectionMaterialState = "missing" | "regular-file" | "directory" | "symlink" | "other";

export interface ProjectInspectionMaterial {
  path: string;
  state: ProjectInspectionMaterialState;
  size?: number;
  sha256?: string;
}

export interface ProjectInspectionSnapshot {
  schemaVersion: 1;
  id: string;
  algorithm: "sha256";
  projectLocatorDigest: string;
  materials: ProjectInspectionMaterial[];
}

export interface ProjectFindingsReport {
  schemaVersion: 1;
  projectRoot: string;
  inspectionSnapshot: ProjectInspectionSnapshot;
...
type PendingFinding = Omit<ProjectFinding, "sourceSnapshotId">;
...
function fileState(path: string): { regular: boolean; symlink: boolean; directory: boolean; size?: number } {
  try {
    ...
    return { regular: false, symlink: false, directory: false };
  }
}
...
function isRegularNonSymlinkDirectory(path: string): boolean {
  return fileState(path).directory;
}

function stableFindingId(ruleId: string, evidence: ProjectFindingEvidence[]): string {
  const canonicalEvidence = evidence
    .map((item) => `${item.path}:${item.detail}:${item.materialDigest ?? "no-material-digest"}`)
    .sort()
    .join("|");
  return `finding-v1:${sha256Bytes(`${ruleId}|${canonicalEvidence}`)}`;
}

function finding(input: Omit<PendingFinding, "id">): PendingFinding {
...
function materialDigestFromHash(hash: string): string {
  return `sha256:${hash}`;
}

function inspectMaterial(root: string, relativePath: string, options: { hashContents?: boolean; maxBytes?: number } = {}): ProjectInspectionMaterial {
  const fullPath = resolve(root, relativePath);
  const state = fileState(fullPath);
  if (!existsSync(fullPath)) return { path: relativePath, state: "missing" };
  if (state.symlink) return { path: relativePath, state: "symlink" };
  if (state.directory) return { path: relativePath, state: "directory" };
  if (!state.regular) return { path: relativePath, state: "other" };

  const material: ProjectInspectionMaterial = { path: relativePath, state: "regular-file", size: state.size };
  if (options.hashContents && (options.maxBytes === undefined || (state.size ?? Number.POSITIVE_INFINITY) <= options.maxBytes)) {
    material.sha256 = hashFile(fullPath);
  }
  return material;
}

function createInspectionSnapshot(root: string): ProjectInspectionSnapshot {
  const materials: ProjectInspectionMaterial[] = [
    inspectMaterial(root, "package.json", { hashContents: true, maxBytes: MAX_PACKAGE_BYTES }),
    ...LOCKFILES.map((path) => inspectMaterial(root, path)),
    inspectMaterial(root, ".git"),
    ...SENSITIVE_ROOT_FILES.map((path) => inspectMaterial(root, path)),
    inspectMaterial(root, ".gitignore", { hashContents: true, maxBytes: MAX_GITIGNORE_BYTES }),
    inspectMaterial(root, "CLAUDE.md", { hashContents: true, maxBytes: MAX_AGENT_GUIDANCE_BYTES }),
    inspectMaterial(root, "AGENTS.md", { hashContents: true, maxBytes: MAX_AGENT_GUIDANCE_BYTES }),
  ].sort((a, b) => a.path.localeCompare(b.path));

  const projectLocatorDigest = sha256Bytes(realpathSync(root));
  const canonical = JSON.stringify({ schemaVersion: 1, projectLocatorDigest, materials });
  return {
    schemaVersion: 1,
    id: `findings-snapshot-v1:${sha256Bytes(canonical)}`,
    algorithm: "sha256",
    projectLocatorDigest,
    materials,
  };
}
...
function inspectPackageManifest(root: string): PendingFinding[] {
  ...
  const packageMaterialDigest = materialDigestFromHash(packageHash);
  let parsed: unknown;
  try {
    ...
    return [finding({
      ...
      evidence: [{ path: "package.json", detail: "JSON parsing failed", materialDigest: packageMaterialDigest }],
  ...
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    return [finding({
      ...
      evidence: [{ path: "package.json", detail: "top-level JSON value is not an object", materialDigest: packageMaterialDigest }],
      nextStep: "Restore package.json to a normal object-shaped Node package manifest.",
    })];
  }

  const manifest = parsed as Record<string, unknown>;
  const result: PendingFinding[] = [];
  ...
  if (lockfiles.length === 0 && hasDeclaredInstallDependencies(manifest)) {
    result.push(finding({
      ...
      evidence: [{ path: "package.json", detail: "installable dependencies declared while no supported Node lockfile is present", materialDigest: packageMaterialDigest }],
  ...
  if (typeof scripts === "object" && scripts !== null && !Array.isArray(scripts)) {
    for (const [name, command] of Object.entries(scripts as Record<string, unknown>).sort(([a], [b]) => a.localeCompare(b))) {
      ...
      const commandDigest = materialDigestFromHash(sha256Bytes(command));
      result.push(finding({
        ...
        evidence: detectors.map((detail) => ({ path: `package.json#scripts.${name}`, detail, materialDigest: commandDigest })),
...
function inspectUnsafeSensitiveRootPaths(root: string): PendingFinding[] {
...
function inspectSensitiveRepositoryRoot(root: string): PendingFinding[] {
  if (!isRegularNonSymlinkDirectory(resolve(root, ".git"))) return [];
  ...
  const ignoreHash = ignoreState?.regular && (ignoreState.size ?? 0) <= MAX_GITIGNORE_BYTES ? hashFile(ignorePath) : undefined;
  ...
  return [finding({
    ...
    evidence: [
      ...
      { path: ".gitignore", detail: ignoreDetail, ...(ignoreHash ? { materialDigest: materialDigestFromHash(ignoreHash) } : {}) },
    ],
...
function inspectAgentGuidance(root: string): PendingFinding[] {
  ...
  return [finding({
    ...
    evidence: [
      { path: "CLAUDE.md", detail: `sha256:${claudeHash}`, materialDigest: materialDigestFromHash(claudeHash) },
      { path: "AGENTS.md", detail: `sha256:${agentsHash}`, materialDigest: materialDigestFromHash(agentsHash) },
    ],
...
export function scanProjectFindings(projectPath: string = process.cwd()): ProjectFindingsReport {
  ...
  const inspectionSnapshot = createInspectionSnapshot(root);
  const pending = [
  ...
  const findings: ProjectFinding[] = pending.map((item) => ({ ...item, sourceSnapshotId: inspectionSnapshot.id }));
  ...
  return {
    ...
    limitations: [
      ...
      "The report is current only for its inspectionSnapshot; later consumers must re-run findings and compare snapshot IDs before treating stored evidence as current.",
      "The inspection snapshot binds only bounded material Livariant actually inspects plus safe file-state metadata; sensitive-file contents are not read or hashed.",
      ...
      "No finding or snapshot grants mutation, Runtime, Release, or publication Authority.",
