...
export type DiagnosticEvent = ObservedDiagnosticEvent | AvoidedDiagnosticEvent | EstimatedDiagnosticEvent;
export type DiagnosticRange = { start?: string; end?: string };
...
export function validateDiagnosticEvent(event: DiagnosticEvent): void {
  if (!event.id.trim()) throw new Error("Diagnostic event id must not be empty.");
  parseTimestamp(event.timestamp, "Diagnostic event timestamp");
  if (event.kind === "observed") {
    if (!event.source || typeof event.source !== "object") throw new Error("Observed evidence source must be present.");
    if (event.source.kind !== "provider" && event.source.kind !== "runtime") throw new Error("Observed evidence source kind must be provider or runtime.");
    if (!event.source.id.trim() || !event.source.version.trim()) throw new Error("Observed evidence requires source id and version.");
    for (const field of TOKEN_FIELDS) {
      const value = event.usage[field];
      if (value !== undefined) assertFiniteNonNegativeInteger(value, `Observed ${field}`);
    }
    return;
  }
  if (event.kind === "avoided") {
    ...
    if (event.usedTokens > event.consideredTokens) throw new Error("Avoided usedTokens must not exceed consideredTokens.");
    if (!event.reason.trim()) throw new Error("Avoided event reason must not be empty.");
    return;
  }
  assertFiniteNonNegativeInteger(event.estimatedTokens, "Estimated estimatedTokens");
  if (!event.method.id.trim() || !event.method.version.trim()) throw new Error("Estimated events require method id and version.");
  if (!event.reason.trim()) throw new Error("Estimated event reason must not be empty.");
}

export function diagnosticRangeForPreset(preset: DiagnosticPreset, now: string | Date = new Date()): DiagnosticRange {
  if (preset === "all") return {};
  const endMs = now instanceof Date ? now.getTime() : parseTimestamp(now, "Range now");
  if (!Number.isFinite(endMs)) throw new Error("Range now must be a valid timestamp.");
  const days = Number.parseInt(preset, 10);
  return { start: new Date(endMs - days * 86400000).toISOString(), end: new Date(endMs).toISOString() };
}
...
export function aggregateDiagnosticEvents(events: readonly DiagnosticEvent[], range: DiagnosticRange = {}): DiagnosticAggregate {
  ...
  const result: DiagnosticAggregate = {
    range: { ...range }, eventCount: 0,
    observed: { eventCount: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, reasoningTokens: 0, totalTokens: 0, knownFieldCount: 0, unknownFieldCount: 0 },
    avoided: { eventCount: 0, contextTokens: 0 },
    estimated: { eventCount: 0, tokens: 0, byConfidence: { low: 0, medium: 0, high: 0 }, byMethod: {} },
  };
  for (const event of events) {
    validateDiagnosticEvent(event);
    if (!inRange(event.timestamp, normalizedRange)) continue;
    result.eventCount += 1;
    if (event.kind === "observed") {
      result.observed.eventCount += 1;
      for (const field of TOKEN_FIELDS) {
        const value = event.usage[field];
        if (value === undefined) result.observed.unknownFieldCount += 1;
        else { result.observed.knownFieldCount += 1; result.observed[field as TokenField] += value; }
      }
      continue;
    }
    if (event.kind === "avoided") { result.avoided.eventCount += 1; result.avoided.contextTokens += event.consideredTokens - event.usedTokens; continue; }
