...
const ACCEPTANCE_STATE_RELATIVE_PATH: [&str; 2] = ["operator-broadcast", "acceptance-state.json"];
...
fn acceptance_state_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(root
        .join(ACCEPTANCE_STATE_RELATIVE_PATH[0])
        .join(ACCEPTANCE_STATE_RELATIVE_PATH[1]))
}

pub fn refresh_once(app: &AppHandle) -> Result<(), String> {
    let endpoint = PRODUCTION_ENDPOINT
        .ok_or_else(|| "Operator broadcast production endpoint is not configured.".to_owned())?;
    let response_bytes = fetch_bounded(endpoint)?;
    let state_path = acceptance_state_path(app)?;
    accept_bounded_response_and_record_notices(app, &response_bytes, &state_path, now_ms()?)?;
    Ok(())
}
...
mod tests {
    ...
    fn acceptance_state_stays_in_app_managed_storage() {
        assert_eq!(ACCEPTANCE_STATE_RELATIVE_PATH, ["operator-broadcast", "acceptance-state.json"]);
    }
}
